/**
 * Source Control Sync Panel Component
 * Workspace-level panel for managing source control synchronization
...
interface SourceControlSyncPanelProps {
  workspaceId: string;
...
export function SourceControlSyncPanel({
  workspaceId,
  sourceControlToken,
  sourceControlRepo,
  onCommitAll,
  ...
  const loadUncommittedChanges = async () => {
    ...
    try {
      // This would call source-control-sync-service.getUncommittedChanges
  ...
  useEffect(() => {
    if (workspaceId && sourceControlToken && sourceControlRepo) {
      loadUncommittedChanges();
    }
  }, [workspaceId, sourceControlToken, sourceControlRepo]);
  ...
  const handlePull = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to pull from source control');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePush = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to push to source control');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="bg-white rounded-lg shadow-md border border-gray-200">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900">Source Control Sync Status</h3>

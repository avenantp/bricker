/**
 * Source Control History Panel Component
...
import type { SourceControlCommit } from '../../types/dataset';

interface SourceControlHistoryPanelProps {
  datasetId: string;
  workspaceId: string;
  onLoadHistory: (workspaceId: string, datasetId: string) => Promise<SourceControlCommit[]>;
  onRevertToCommit?: (commitSha: string) => Promise<void>;
  onViewCommitDiff?: (commitSha: string) => void;
}

export function SourceControlHistoryPanel({
  datasetId,
  workspaceId,
  onLoadHistory,
  onRevertToCommit,
  onViewCommitDiff,
}: SourceControlHistoryPanelProps) {
  const [commits, setCommits] = useState<SourceControlCommit[]>([]);
  ...
  const loadHistory = async () => {
    ...
    try {
      const history = await onLoadHistory(workspaceId, datasetId);
      setCommits(history);
    } catch (err: any) {
        ...
        {loading && commits.length === 0 ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Commits will appear here after you save changes to source control
            </p>
          </div>
        ) : (

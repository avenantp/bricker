...
CREATE TABLE IF NOT EXISTS companies (
  ...
  company_type VARCHAR NOT NULL CHECK (company_type IN ('individual', 'organization')),

  -- Subscription
  subscription_tier VARCHAR DEFAULT 'free' CHECK (subscription_tier IN ('free', 'pro', 'enterprise')),
  subscription_status VARCHAR DEFAULT 'active' CHECK (subscription_status IN ('active', 'suspended', 'cancelled')),
...
CREATE TABLE IF NOT EXISTS subscription_plans (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name VARCHAR NOT NULL UNIQUE,
  tier VARCHAR NOT NULL CHECK (tier IN ('free', 'pro', 'enterprise')),
...
CREATE TABLE IF NOT EXISTS users (
  ...
  -- Role in company
  company_role VARCHAR NOT NULL DEFAULT 'member' CHECK (company_role IN ('admin', 'member')),
...
-- Company Users (company membership)
...
-- Environments
CREATE INDEX IF NOT EXISTS idx_environments_company ON environments(company_id);
CREATE INDEX IF NOT EXISTS idx_environments_project ON environments(project_id);
CREATE INDEX IF NOT EXISTS idx_environments_workspace ON environments(workspace_id);

-- Source Code Commits
CREATE INDEX IF NOT EXISTS idx_source_control_commits_company ON source_control_commits(company_id);
...
COMMENT ON TABLE companies IS 'Multi-tenant root - all resources belong to a company';
COMMENT ON TABLE users IS 'User profiles extending Supabase auth.users with company association';
COMMENT ON TABLE company_users IS 'Company membership with roles';
